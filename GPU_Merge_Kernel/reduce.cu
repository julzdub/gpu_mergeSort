...
double * GPUPrep(double *data, int n1, int *n2)
{
    *n2 = nextPowerOfTwo(n1);
    double *ret = (double *) malloc( *n2 * sizeof(double) );
    memset(ret, 0, *n2 * sizeof(double) );
    memcpy(ret, data, n1 * sizeof(double));
...
double * fillArray(int n, int upbound)
{
   int i;
...
double cpuReduce(double *h_in, int n)
{
    double s = 0;
    int i;
    for(i = 0; i < n; i ++)
...
__global__ void reduce2(double *in, double *out, int n)
{
...
__global__ void reduce3(double *in, double *out, int n)
{
	extern __shared__ double sdata[];
	...
	sdata[tid] = (i < n ) ? in[i] : 0;
    sdata[tid+blockDim.x] = (i+blockDim.x < n) ? in[i+blockDim.x] : 0;
	__syncthreads();

    int flag = 0;
	
    for(unsigned int s = blockDim.x; flag == 0; s/=2){
...
//
void runCUDA( double *arr, int  n_old, int tile_width)
{    
   // set up host memory
   double *h_in, *h_out, *d_in, *d_out;
   int n = 0;
   h_in = GPUPrep(arr, n_old, &n); //Make input size power of 2.
   h_out = (double *)malloc(MAXDRET * sizeof(double));
   memset(h_out, 0, MAXDRET * sizeof(double));
   ...
   int num_block = (ceil(n / (double)tile_width));
   ...
   cudaMalloc((void**)&d_out, sizeof(double) * MAXDRET);
   cudaMemset(d_out, 0, sizeof(double) * MAXDRET);
   ...
   int num_in = n, num_out = ceil((double)n / tile_width);
   double *temp;
   ...
   while( 1 )
   {
       reduce3<<<grid, block, tile_width * sizeof(double)>>>(d_in, d_out, num_in);
       ...
       {
           num_in = num_out;
           num_out = ceil((double)num_out / tile_width);
       ...
       else
       {
           ...
           cudaMemcpy(h_out, d_out, sizeof(double) * num_out, cudaMemcpyDeviceToHost);
           break;
       }
    }//end of while
...
int main(int argc, char *argv[])
{
    ...
    if ( ! n || ceil(n / (double)tile_width) > 262144 )
    {
