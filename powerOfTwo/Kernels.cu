...
__global__ void mergeKernel(int* in, int* out, int dataSize, int tile_size){
   ...
   //unsigned int tid = threadIdx.x;
   unsigned int i = ((blockIdx.x*blockDim.x)*tile_size * 2)+ threadIdx.x * (tile_size*2); //change based on number of elements imported by thread
...
{  
   ...
   if(isVal == -1){
    ...
    int * result = runMerge(keyResult, dataSize, 1024, time);
...
int * runMerge(int * arr, int n, int tile_width, float time){
   ...
   while(t_width < n/2){
      //puts("\n\n RAN \n\n");
      //printf("T: %d\n", t_width);
      num_threads = n/(t_width*2);
      if(num_threads >= 1024){
         num_block = ceil(num_threads/1024.0);
         num_threads = 1024;
      }else{
         num_block = 1;
      }
      printf("Blocks: %d\n", num_block);
      printf("Threads: %d\n", num_threads);
      mergeKernel<<<num_block,num_threads>>>(d_in, d_out, n, t_width);
      check_cuda_errors(__FILE__, __LINE__);
      t_width *= 2;
   ...
   merge(h_out, 0, (n-1)/2, n-1);

   now =  clock();
    	
   // measure the time spent on CPU
   float timeCPU = 0;
   timeCPU = timeCost(then, now);

   //printf("Done! Time cost is. %f\n", time2/1000);
   printf("GPU time cost in seconds: %f\n", (time+time2+timeCPU)/1000);
