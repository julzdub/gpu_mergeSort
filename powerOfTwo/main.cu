...
int main(int argc, char *argv[])
{
	...
	printf("Pad:%d/n", wPadding);
	int *cpuArray = fillArray(arraySize, 200, wPadding);
	...
	printOutput(output_CPU, cpuArray, wPadding);

	
	//Call kernel setup
	printf("Timing GPU implementation…\n");
	int * result = runCUDA(gpuArray, wPadding);

    printOutput(output_GPU, gpuArray, wPadding);
